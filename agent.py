...
if not person_conf:
    person_conf = 0.65
    print(f"PERSON_CONFIDENCE_THR env var not specified. Defaulting to {person_conf}. You can adjust the confidence by providing the env var in the agent configuration")
if not tv_conf:
    tv_conf = 0.7
    print(f"TV_CONFIDENCE_THR env var not specified. Defaulting to {tv_conf}. You can adjust the confidence by providing the env var in the agent configuration")

print(f"Agent running! Add a pipeline using Home Assistant as video source to start automating your home with vision!")
